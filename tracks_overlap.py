...
for tr in range(N0_TRACKS,1,-1):
    print('\nlooking for multiple of',tr,'overlapping')
    ...
    common_points.update({tr:X[index_to_be_deleted]})   # store the common points in dictionary

    if len(common_points[tr]>0):                        # detect how many tracks need to
        N0_TRACKS_TO_BE_DISPLAYES+=1                    # displayed, where an overlapping was detected

    X_new=np.delete(X,index_to_be_deleted,axis=0)
    index_to_be_deleted=[]
    X=X_new.copy()
    LEN_ALL_POINTS = len(X)

common_points.update({1:X})     # nearest neighbor with single points not possible, hence
...
plt.ylabel('lateral')
plt.xlabel('longitudinal')
color_map = discrete_cmap(N0_TRACKS_TO_BE_DISPLAYES, 'gnuplot')

x=[]
y=[]
point_color=[]
for i in range(N0_TRACKS_TO_BE_DISPLAYES,0,-1):
    x+=common_points[i][:, 1].tolist()
    y+=common_points[i][:, 0].tolist()
    point_color+= [i] * len(common_points[i])
